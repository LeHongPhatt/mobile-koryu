...
export const useKey = () => {
  ...
  const getPwd = async () => {
    try {
      ...
      if (credentials) {
        ...
        return {account: credentials.username, password: credentials.password};
      }
      return null;
    } catch (error) {
      return null;
  ...
  const resetPwd = async (service?: string) => {
    if (service) {
  ...
  return {savePwd, getPwd, resetPwd, saveKeyStore, getKeyStore};
};
